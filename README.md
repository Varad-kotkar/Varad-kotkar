
![Python](https://img.shields.io/badge/Python-Expert-blue)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-Projects-green)
![OpenCV](https://img.shields.io/badge/OpenCV-Computer%20Vision-red)
![PyTorch](https://img.shields.io/badge/PyTorch-Deep%20Learning-orange)
![Power BI](https://img.shields.io/badge/PowerBI-Analytics-yellow)
# 👋 Hi, I'm Varad Kotkar

<img align="right" alt="AI Coding" width="350" src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExdHJvNXVtdzFza2xudGZ1NWh6d2g4c3h3M2h0dDdxY2U3YWFxOGFmMCZlcD12MV9naWZzX3NlYXJjaCZjdD1n/l0HlNaQ6gWfllcjDO/giphy.gif"/>

### 🧠 AI • Machine Learning • Computer Vision

🎓 B.Tech CSE @ MIT ADT University

📍 Ahilyanagar, India

🚀 Building intelligent systems using Python, OpenCV & Deep Learning

---

## ⚡ Skill Matrix

| Skill | Level |
|---------|---------|
| Python | ██████████ |
| Machine Learning | █████████░ |
| Computer Vision | ████████░░ |
| Data Analytics | ████████░░ |
| Power BI | ████████░░ |
| Deep Learning | ██████░░░░ |

---

## 🚀 Featured Projects

### 🚗 Driver Drowsiness Detection
Real-time AI system that detects driver fatigue using Computer Vision and alerts users to prevent accidents.

### 🤖 AI-Based Fake Review Detection
Machine Learning model that identifies suspicious and fake online reviews using text analysis and classification techniques.

### ✋ Hand Gesture Recognition
Computer Vision application that recognizes hand gestures and converts them into machine-understandable commands.

### 📊 Customer Segmentation Analysis
Clustering-based analytics solution that groups customers by behavior to support targeted business decisions.

---

## 🎯 Currently Learning

```text
[████████░░] Deep Learning

[██████░░░░] MLOps

[█████░░░░░] Generative AI
